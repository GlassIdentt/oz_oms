...
function updateOrderCounter() {
	...
	// console.log(
	// 	"counterItemsMapping 확인:",
	// 	typeof counterItemsMapping,
	// 	counterItemsMapping
	// );

	if (
		typeof counterItemsMapping !== "undefined" &&
		Array.isArray(counterItemsMapping) &&
		counterItemsMapping.length > 0
	) {
				...
				if ($element.length === 0) {
					...
					if (container) {
						...
						if (directElement && directElement.tagName === "DIV") {
							...
							console.log(
								".bottom_stats_container 내부에서 div 요소 찾음:",
								item.id
							);
						} else {
				...
				if ($element.length > 0) {
					...
					setTimeout(function () {
						...
						if (!isVisible || actualText === "") {
							console.warn(
								"요소가 화면에 보이지 않습니다. 강제로 다시 렌더링합니다:",
								item.id
							);
							$element.css({
								display: "none",
				...
				if (mutation.type === "childList") {
					mutation.addedNodes.forEach(function (node) {
						if (
							...
							(node.id === "T_S_AMT" ||
								node.id === "T_B_AMT" ||
								node.classList.contains("stats_value"))
						) {
							shouldUpdate = true;
						}
					});
