...
<script>
	...
	function loadCategoryView(category) {
		...
		// 현재 페이지의 경로를 기준으로 URL 생성
		// 현재 경로가 /oz_oms/인 경우를 고려
		let url;
		const pathname = window.location.pathname;
		
		if (pathname.indexOf('/index.php') !== -1) {
			// index.php가 URL에 포함된 경우
			url = window.location.origin + pathname.replace(/\/[^\/]*$/, '') + '/category';
		} else {
			// index.php가 없는 경우 (기본 경로)
			url = window.location.origin + '/oz_oms/index.php/welcome/category';
		}
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState === 4) {
				if (xhr.status === 200) {
					...
					contentsArea.innerHTML = '<div style="padding: 20px; color: red;">파일을 불러오는 중 오류가 발생했습니다.<br>상태 코드: ' + xhr.status + '<br>URL: ' + url + '<br>현재 경로: ' + window.location.pathname + '</div>';
				}
			}
		};
		...
		xhr.onerror = function() {
			...
			contentsArea.innerHTML = '<div style="padding: 20px; color: red;">네트워크 오류가 발생했습니다. URL을 확인해주세요: ' + url + '</div>';
		};
