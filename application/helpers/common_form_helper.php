...
/**
 * Proc_So_Aloc_Section_T_List_Json 프로시저를 호출하여 데이터를 가져오는 함수
 * 
 * @param string $t_date 날짜 (YYYYMMDD 형식)
 * @param string $aloc_type 배차유형
 * @param string $so_mode 상품모드
 * @param bool $debug 디버그 모드 (기본값: false)
 * @return array Grid 데이터 배열
 */
if (!function_exists('get_section_allocation_t_list_data')) {
    function get_section_allocation_t_list_data($t_date = '', $aloc_type = '', $so_mode = '', $debug = false)
    {
        $CI =& get_instance();
        $grid_data = array();
        $query_result = null; // 디버깅용
        ...
        try {
            ...
            // T_DATE 최종 검증 (8자리 숫자 형식)
            if (strlen($t_date) != 8 || !is_numeric($t_date)) {
                $t_date = date('Ymd');
            }

            // 프로시저 실행
            $sql = "EXEC [dbo].[Proc_So_Aloc_Section_T_List_Json] @T_DATE = ?, @ALOC_TYPE = ?, @SO_MODE = ?";
                ...
                if (!empty($result)) {
                    ...
                    if ($row_vars !== null) {
                        ...
                        if (isset($row_vars['JsonData'])) {
                            ...
                            // NULL 체크
                            if (is_null($json_string)) {
                                log_message('error', 'Proc_So_Aloc_Section_T_List_Json: JsonData가 NULL입니다.');
                                $grid_data = array();
                            } 
                            // 빈 문자열 체크
                            elseif ($json_string === '' || $json_string === false || empty($json_string)) {
                                log_message('error', 'Proc_So_Aloc_Section_T_List_Json: JsonData가 빈 문자열입니다.');
                                $grid_data = array();
                            } 
                            // JSON 문자열 디코딩
                            else {
                                ...
                                if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                                    ...
                                    // JSON 디코딩 실패 시 빈 배열
                                    log_message('error', 'Proc_So_Aloc_Section_T_List_Json: JSON 디코딩 실패: ' . json_last_error_msg() . ' | JsonData 값: ' . substr($json_string, 0, 200));
                                    $grid_data = array();
                                }
                            }
                        } 
            ...
            // 에러 발생 시 로그 기록 및 빈 배열 반환
            log_message('error', 'Proc_So_Aloc_Section_T_List_Json 프로시저 실행 실패: ' . $e->getMessage());
            $grid_data = array();
        }
        ...
        // 디버깅 출력
        if ($debug) {
            echo "<!-- DEBUG: Proc_So_Aloc_Section_T_List_Json 결과 출력 시작 -->\n";
            echo "<!-- grid_data 타입: " . gettype($grid_data) . " -->\n";
            echo "<!-- grid_data 개수: " . (is_array($grid_data) ? count($grid_data) : 'N/A') . " -->\n";
            echo "<!-- 파라미터: T_DATE={$t_date}, ALOC_TYPE={$aloc_type}, SO_MODE={$so_mode} -->\n";
            if (is_array($grid_data) && count($grid_data) > 0) {
                echo "<!-- grid_data 첫 번째 항목: " . htmlspecialchars(print_r($grid_data[0], true)) . " -->\n";
            } else {
                echo "<!-- grid_data: 빈 배열 또는 데이터 없음 -->\n";
            ...
            echo "<!-- DEBUG: Proc_So_Aloc_Section_T_List_Json 결과 출력 끝 -->\n";
        }
